...
export function DocumentChecklist({ place, members, lang, t, nested = false }: { place: PlaceView; members: MemberTravelChecklist[]; lang: Lang; t: Messages; nested?: boolean }) {
  ...
  return <section className={`document-checklists${nested ? " nested" : ""}`} aria-labelledby="document-checklists-title">
    <div className="document-checklists-heading"><div><Heading id="document-checklists-title">{t.documentChecklistTitle}</Heading><p>{t.documentChecklistNotice}</p></div><button type="button" onClick={() => copy("all", members)}><Copy aria-hidden="true" />{copied === "all" ? t.checklistCopied : t.copyAllChecklists}</button></div>
    <div className="member-checklists">{members.map((member) => <article key={member.id} className="member-checklist">
      <div className="member-checklist-heading"><h4>{memberLabel(member, t)}</h4><button type="button" aria-label={`${t.copyChecklist}: ${memberLabel(member, t)}`} onClick={() => copy(member.id, [member])}>{copied === member.id ? <Check aria-hidden="true" /> : <Copy aria-hidden="true" />}{copied === member.id ? t.checklistCopied : t.copyChecklist}</button></div>
